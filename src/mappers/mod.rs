...
impl Mapper {
    pub fn new( id: u16, prg_rom: Vec<u8>, chr_rom: Vec<u8>, prg_ram: Vec<u8> ) -> RefCell<Box<Mapper>> {
        match id {
...
impl MemSegment for Mapper {
    fn read(&self, idx: u16) -> u8 { self.prg_read( idx ) }
    fn write(&mut self, idx: u16, val: u8) { self.prg_write( idx, val ) }
}